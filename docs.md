...
| Браузер | Microsoft Edge, Google Chrome, а также Яндекс Браузер актуальной корпоративной версии |
...
Для целевого контура требуется отдельное приемочное подтверждение работоспособности интерфейса, 3D-визуализации, импорта и экспорта в Яндекс Браузере как в потенциальном корпоративном браузере по умолчанию.
...
| Специалист ИБ | имеет доступ только на чтение к журналам событий, аудиту действий пользователей, техническим логам и статусам операций |
...
| Сквозная авторизация пользователей между приложением и смежными ИС | не требуется и не реализуется в текущей архитектуре |
...
Под сквозной авторизацией в данном документе понимается передача пользовательской идентичности в смежные корпоративные системы и выполнение операций в них от имени пользователя. Для текущей версии приложения такой сценарий не требуется: система не содержит обязательных онлайн-интеграций с бизнес-сервисами, не обращается к `Blitz`, `Keycloak` и аналогичным системам напрямую и работает с файлами, которые пользователь загружает вручную или читает из согласованных сетевых ресурсов. Поэтому в целевой архитектуре достаточно внешней корпоративной аутентификации на уровне портала, reverse proxy или IAM-контура доступа.
...
8. тест импорта, экспорта и 3D-визуализации на целевых рабочих местах, включая Яндекс Браузер как потенциальный корпоративный браузер;
