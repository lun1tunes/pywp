    ...
    with st.expander("Параметры солвера", expanded=False):
        if show_solver_help:
            with st.popover("Что означают параметры солвера", icon=":material/tune:"):
                st.markdown(
                    "- Солвер всегда минимизирует итоговую `MD`.\n"
                    "- На криволинейных участках используется максимально допустимый `ПИ BUILD`.\n"
        ...
        st.caption(
            "Солвер использует фиксированную цель `Минимизировать итоговую MD` "
            "и максимальный BUILD ПИ. Пользователь выбирает метод решателя и допустимое число авто-рестартов."
        )
        st.selectbox(
            "Метод решателя",
