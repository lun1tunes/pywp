...
def _render_target_table_help() -> None:
    ...
    _render_example(
        ...
        "Пилот задаётся отдельной записью с тем же базовым именем и суффиксом `_PL` или `PL`. Используйте точки `S`, `PL1`, `PL2`, ... без пропусков. Точек пилота может быть сколько угодно: от `PL1` до `PLN`.",
    )
    _render_example(
...
def _render_source_input() -> None:
    ...
    with st.expander("Таблица точек целей", expanded=True):
        st.caption(
            ...
            "Допустимы `S` и `S1`; регистр меток не важен."
        )
        with st.expander(
