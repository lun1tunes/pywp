...
from pywp.eclipse_welltrack import WelltrackParseError, WelltrackRecord, parse_welltrack_text
...
def _build_config_form() -> TrajectoryConfig:
    ...
    pos_tolerance_m = c3.number_input("Допуск по позиции, м", key="wt_pos_tolerance_m", min_value=0.1, step=0.1)
    entry_inc_target_deg = c4.number_input(
        "Целевой INC входа", key="wt_entry_inc_target_deg", min_value=70.0, max_value=89.0, step=0.5
    )
    entry_inc_tolerance_deg = c5.number_input(
        "Допуск INC", key="wt_entry_inc_tolerance_deg", min_value=0.1, max_value=5.0, step=0.1
    )

    d1, d2, d3 = st.columns(3, gap="small")
    dls_build_min = d1.number_input("Мин DLS BUILD", key="wt_dls_build_min", min_value=0.1, step=0.1)
    dls_build_max = d2.number_input("Макс DLS BUILD", key="wt_dls_build_max", min_value=0.1, step=0.1)
    kop_min_vertical_m = d3.number_input(
        "Мин VERTICAL до KOP, м", key="wt_kop_min_vertical_m", min_value=0.0, step=10.0
    )
    ...
    with st.expander("TURN и оптимизация", expanded=False):
        ...
        objective_mode = e1.selectbox(
            ...
            key="wt_objective_mode",
            format_func=lambda key: OBJECTIVE_OPTIONS[str(key)],
        )
        turn_solver_mode = e2.selectbox(
            "TURN solver",
            options=list(TURN_SOLVER_OPTIONS.keys()),
            key="wt_turn_solver_mode",
            format_func=lambda key: TURN_SOLVER_OPTIONS[str(key)],
        )
        e3.number_input("TURN QMC samples", key="wt_turn_solver_qmc_samples", min_value=0, step=4)
        st.number_input("TURN local starts", key="wt_turn_solver_local_starts", min_value=1, step=1)
    ...
    return TrajectoryConfig(
        ...
        turn_solver_qmc_samples=int(st.session_state["wt_turn_solver_qmc_samples"]),
        turn_solver_local_starts=int(st.session_state["wt_turn_solver_local_starts"]),
...
def run_page() -> None:
    ...
    with action_col:
        ...
        parse_clicked = st.button("Прочитать WELLTRACK", type="primary", icon=":material/upload_file:", width="stretch")
        clear_clicked = st.button("Очистить импорт", icon=":material/delete:", width="stretch")
        ...
        if not source_text.strip():
            ...
            with st.status("Чтение и парсинг WELLTRACK...", expanded=True) as status:
                ...
                try:
                    ...
                    status.update(label="Ошибка парсинга WELLTRACK", state="error", expanded=True)
    ...
    selected_names = [name for name in st.session_state.get("wt_selected_names", []) if name in all_names]
    ...
    with st.form("welltrack_run_form", clear_on_submit=False):
        ...
        run_clicked = st.form_submit_button("Рассчитать выбранные скважины", type="primary", icon=":material/play_arrow:")
            ...
            with st.status("Выполняется расчет WELLTRACK-набора...", expanded=True) as status:
                try:
                    ...
                    def on_progress(index: int, total: int, name: str) -> None:
                        progress.progress(int((index / max(total, 1)) * 100), text=f"{index}/{total}: {name}")
                    ...
                    st.session_state["wt_last_error"] = ""
                    st.session_state["wt_last_run_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    ...
                    st.session_state["wt_last_error"] = str(exc)
                    status.write(str(exc))
                    status.update(label="Batch-расчет завершился ошибкой", state="error", expanded=True)
                finally:
                    progress.empty()
    ...
    ok_count = int((summary_df["Статус"] == "OK").sum()) if not summary_df.empty and "Статус" in summary_df else 0
    ...
    render_small_note(f"Последний запуск: {st.session_state.get('wt_last_run_at', '—')}")
    ...
    with tab_single:
        ...
        c2.plotly_chart(dls_figure(stations, dls_limits=cfg.dls_limits_deg_per_30m), width="stretch")
