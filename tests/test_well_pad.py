...
def test_apply_pad_layout_uses_explicit_surface_positions_when_provided() -> None:
    records = [
        _record(
            "W1",
            sx=0.0,
    ...
    plan = PadLayoutPlan(
        pad_id=str(pads[0].pad_id),
        first_surface_x=999.0,
        first_surface_y=999.0,
        first_surface_z=9.0,
        spacing_m=20.0,
        nds_azimuth_deg=90.0,
        surface_positions_by_well_name=(
            ("W2", 2050.0, 510.0, 15.0),
            ("W1", 2000.0, 500.0, 10.0),
        ),
    )
    ...
    assert math.isclose(
        float(updated_by_name["W1"].points[0].y),
        500.0,
        rel_tol=0.0,
        abs_tol=1e-9,
    )
    ...
    assert math.isclose(
        float(updated_by_name["W2"].points[0].y),
        510.0,
        rel_tol=0.0,
        abs_tol=1e-9,
    )
